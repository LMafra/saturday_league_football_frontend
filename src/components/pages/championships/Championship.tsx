...
const Championship: React.FC = () => {
  ...
  const handleCreateRound = async (formData: { name: string; round_date: string; championship_id: string }) => {
    try {
  ...
  if (!championship) return <div className="text-center py-8">Championship not found</div>;
    ...
    <div className="container mx-auto px-4 py-8 mt-24">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-500">
          <div>
            <span className="font-semibold">Created:</span> {format(new Date(championship.created_at), 'dd/MM/yyyy')}
          </div>
          <div>
            <span className="font-semibold">Last Updated:</span> {format(new Date(championship.updated_at), 'dd/MM/yyyy')}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-blue-50 p-6 rounded-lg shadow">
          <h3 className="text-xl font-semibold text-blue-800 mb-2">Total Rounds</h3>
          <p className="text-3xl font-bold text-blue-600">{championship.round_total}</p>
        </div>
        <div className="bg-green-50 p-6 rounded-lg shadow">
          <h3 className="text-xl font-semibold text-green-800 mb-2">Total Players</h3>
          <p className="text-3xl font-bold text-green-600">{championship.total_players}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {championship.players.map((player) => (
            <div key={player.id} className="flex items-center space-x-4 p-3 border rounded-lg hover:bg-gray-50">
