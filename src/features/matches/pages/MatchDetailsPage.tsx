...
const buildTeamBreakdown = (match: Match, teamSide: "team_1" | "team_2"): TeamBreakdown => {
...
const MatchDetailsPage = () => {
  ...
  const team1 = useMemo(() => (match ? buildTeamBreakdown(match, "team_1") : null), [match]);
  const team2 = useMemo(() => (match ? buildTeamBreakdown(match, "team_2") : null), [match]);
          ...
          <div className="grid gap-8 md:grid-cols-3">
            ...
            <div className="flex flex-col items-center justify-center text-center">
              ...
              <div className="mt-2 text-5xl font-bold text-gray-900">
                {team1.goals} <span className="mx-1 text-3xl text-gray-400">x</span> {team2.goals}
              </div>
              {match.winning_team ? (
...
const TeamColumn = ({ team, align }: TeamColumnProps) => {
  ...
  const renderList = (title: string, players: Player[], emptyMessage: string) => (
    <div>
      <h4 className={`text-sm font-semibold text-gray-500 ${textAlign}`}>{title}</h4>
      {players.length > 0 ? (
        ...
        <p className={`mt-2 text-sm text-gray-400 ${textAlign}`}>{emptyMessage}</p>
      )}
    </div>
  );
  ...
  return (
    <div className={`space-y-6 ${textAlign}`}>
      ...
      {renderList("Assistências", team.assists, "Nenhuma assistência registrada.")}
      {renderList("Gols Contra", team.ownGoals, "Nenhum gol contra registrado.")}
      <div>
        <h4 className={`text-sm font-semibold text-gray-500 ${textAlign}`}>Escalação</h4>
