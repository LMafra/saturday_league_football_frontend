...
const PlayerDetailsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<"stats" | "rounds" | "matches">("stats");
  ...
  const aggregatedStats = useMemo(() => {
    ...
    return data.player_stats.reduce(
      (accumulator, stat) => ({
        ...
        goalkeeperMatches: accumulator.goalkeeperMatches + (stat.was_goalkeeper ? 1 : 0),
      }),
      {
        totalGoals: 0,
        totalAssists: 0,
      ...
      <Container>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    Entrou em {format(new Date(data.created_at), "dd MMMM yyyy")}
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="rounded-2xl bg-white shadow-lg">
          <div className="flex border-b">
            {(["stats", "rounds", "matches"] as const).map((tabKey) => (
              ...
              >
                ...
                {tabKey === "rounds" && `Rodadas (${data.rounds?.length ?? 0})`}
                {tabKey === "matches" &&
          ...
          <div className="space-y-6 p-6">
            ...
            {activeTab === "rounds" && (
              <div>
                <h2 className="text-xl font-semibold text-gray-900">Rodadas</h2>
            ...
            {activeTab === "matches" && (
              <div>
                <h2 className="text-xl font-semibold text-gray-900">Estatísticas por Partida</h2>
                {data.player_stats && data.player_stats.length > 0 ? (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">Time #{stat.team_id}</p>
                          </div>
                          {stat.was_goalkeeper && (
                        ...
                        <dl className="mt-4 grid grid-cols-3 gap-2 text-center">
                          ...
                          <div>
                            <dt className="text-xs text-gray-500">Assistências</dt>
                            <dd className="text-xl font-semibold text-blue-600">
                              {stat.assists}
                            </dd>
                          </div>
                          <div>
                            <dt className="text-xs text-gray-500">Gols Contra</dt>
                  ...
                  <p className="mt-4 text-gray-500">Sem estatísticas registradas.</p>
                )}
              </div>
            )}
          </div>
